...
const calculator = () => {
    ...
    function initFromLocalStorage() {
        for (let key in personData) {
            if (key === "gender" || key === "ratio") {
                const dataAttrElems = document.querySelectorAll(
                    `[data-${key}]`
                );
    ...
    window.addEventListener("click", (e) => {
        ...
        if (
            t.closest("#activity") &&
            t.classList.contains("calculating__choose-item")
        ) {
            changeValueWithDataAttr(
                activityWrapper,
                t,
                "data-ratio",
                personData
    ...
    function changeValueWithDataAttr(parent, element, attr, obj) {
        ...
        removeClassFromArr(children, "calculating__choose-item_active");
    ...
    function calculateTotal(obj) {
        if (checkKeysIsTrue(personData)) {
            const { weight, height, age, ratio } = obj;
            let total = 0;
